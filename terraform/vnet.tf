...
resource "azurerm_subnet" "vm_subnet_01" {
    for_each = { for vnet in local.flatten_vnet_details : "${vnet.subnet_name}-${var.environment}-${var.project}-${vnet.location}-${vnet.instance}" => vnet }
    
    name                 = "${each.value.subnet_name}-${var.environment}-${var.project}-${each.value.location}-${each.value.instance}"
    resource_group_name  = azurerm_resource_group.vm_rg_01.name
    virtual_network_name = azurerm_virtual_network.vm_vnet_01["${each.value.vnet_name}-${var.environment}-${var.project}-${each.value.location}-${each.value.instance}"].name
